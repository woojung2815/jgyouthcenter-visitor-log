...
if st.session_state.is_admin and st.session_state.page == 'admin':
    ...
    if not df.empty:
        ...
        with st.expander("🔍 상세 필터링 설정", expanded=True):
            ...
            with f_col1: date_range = st.date_input("날짜 범위", [df['일시'].min().date(), df['일시'].max().date()])
            with f_col2: selected_gender = st.multiselect("성별", options=["남성", "여성"], default=["남성", "여성"])
            
            f_col3, f_col4 = st.columns(2)
            with f_col3: selected_ages = st.multiselect("연령대", options=AGE_GROUPS, default=AGE_GROUPS)
            with f_col4: selected_purposes = st.multiselect("이용 목적", options=PURPOSES, default=PURPOSES)
